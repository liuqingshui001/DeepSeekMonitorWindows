...
### 显示分类自定义
- 可自由开关 DeepSeek API、CPU、GPU、内存、磁盘、网络等数据分类
- 设置页勾选/取消勾选即可，自动记忆选择
...
1. 下载 `DeepSeekMonitorWindows_1.3.0_x64-setup.exe`
2. 双击运行安装程序，可自由选择安装目录
3. 安装完成后，从开始菜单或桌面快捷方式启动

### 开机自启 (v1.3.0)

自启通过 **Windows 计划任务** 实现，以管理员权限运行，无需 UAC 确认，硬件监控数据完整。在设置中开启「登录时自动启动」即可。
...
在设置页中的「显示分类」区域，可以自由勾选/取消勾选想要查看的数据面板：

- **DeepSeek API**：取消后隐藏 API 监控面板，仅显示硬件监控
- **CPU / GPU / 内存 / 磁盘 / 网络**：取消后对应的传感器卡片将隐藏，只显示勾选的部分

关闭 DeepSeek API 后，窗口自动仅显示硬件监控，简洁专注。
